...
```typescript
// EventQueryPort — resolves event info and ticket type availability
export interface EventQueryPort {
  getEventById(eventId: string): Promise<EventInfo | null>;
  getTicketTypeAvailability(ticketTypeId: string): Promise<TicketTypeAvailability | null>;
...
### 3. QR Code with Checksum Validation
...
### 4. Domain Event Split: Application vs Infrastructure Handlers
...
| `POST` | `/api/tickets/check-in` | Bearer | Organizer/Admin | Check-in with QR code |
| `GET`  | `/api/tickets/event/:eventId/stats` | Bearer | Organizer/Admin | Check-in statistics |
