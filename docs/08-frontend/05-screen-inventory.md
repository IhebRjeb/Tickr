...
| **Rendering + why** | One of the codes in [§0.2](#02-rendering-codes) followed by the reason, in one clause. The reason is part of the deliverable: a rendering choice with no recorded reason is a choice the next person will silently reverse. |
...
4. **This does not change if the token later moves to an `httpOnly` cookie.** Cookie auth would make
   Rule 1 optional for read-only screens such as `/profile`, but the Rule 3 screens stay CSR
   regardless: they poll, they hold a countdown, and they must never be cached.
...
`soldQuantity` is incremented at **hold** time (`event-query.adapter.ts:75`, called from
`reserve-tickets.handler.ts:99`) and **restored** when a hold lapses or is cancelled
(`expire-tickets.handler.ts:93`, `cancel-tickets.handler.ts:105`). A cached availability figure can
therefore be wrong in **both directions**: it can show stock that was taken 40 seconds ago, or hide
...
|---|---|---|---|---|---|---|
| **Landing** | `/` | public | **SSR + ISR 60** (no-city variant) — no `searchParams`, identical for every anonymous visitor, highest-traffic entry point; the persisted city preference is a client overlay and must not enter the cached document | `GET /events/upcoming?city=&country=&page=&limit=` (hero + « À venir »)<br>`GET /events?sortBy=soldTickets&sortOrder=DESC&limit=8` (« Les plus demandés »)<br>`GET /events?sortBy=publishedAt&sortOrder=DESC&limit=8` (« Nouveautés »)<br>`GET /events?category=…&limit=8` per rail<br>⚠ `GET /config/public` **NOT IMPLEMENTED** — not called | `L` per rail · **`∅` cold start — zero published events is a designed screen, not a bug; it is what launch day looks like** · `err` scoped per rail, one failing rail never blanks the page · city overlay refetch after hydration · category rails built from the local `EventCategory` mirror, never fetched · deterministic no-image fallback | **P0** |
...
| **Event detail** | `/events/[id]` | public | **SSR**, `no-store`, **never ISR** — the most shared and most indexed page in the product, so it needs complete HTML and OG tags on the first byte; and availability is a per-fetch snapshot that a shared cache would falsify ([§1.4](#14-caching-and-revalidation)). Sends the bearer token when present, so an owner sees their own `DRAFT` | `GET /events/:id` → `EventDto` incl. `ticketTypes[]` | `L` · `404` for a deleted or never-published event — **say so; do not imply a broken link** · malformed UUID → `ParseUUIDPipe` 400, rendered as `404` · `403` when an anonymous visitor hits a `DRAFT` → rendered as `404` · `CANCELLED`: full-width `danger` banner, purchase bar replaced by text, imagery desaturated · `COMPLETED` · per tier — available / limited (« Plus que 4 places ») / **sold out + « Vérifier à nouveau »** / not-on-sale with the real `salesStartDate` · sheet open · no-image fallback | **P0** |
| **Search results** | `/search` | public | **CSR**, `noindex, follow` — query-driven, changes per keystroke, thin and duplicative for a crawler; SSR would add TTFB and buy nothing | `GET /events/search?q=&page=&limit=` | `L` · idle, no query yet — recent searches + popular · `∅` « Aucun événement pour "jazz" » + a widen action · **empty `q` is a 400**, so the request is not sent below the minimum length · `err` · input debounced ≥ 300 ms to stay under the 3 req/s throttle · ⚠ **no facets** — the endpoint takes only `q`, `page`, `limit`, so refinement chips deep-link to `/events?…` (§7.2) | P1 |
| **Category listing** | `/categories/[category]` | public | **ISR 60 + `generateStaticParams()`** over the 10 `EventCategory` values — finite, known, high-intent SEO landings; nothing here goes stale in a misleading way because the tier-level availability lives on the detail page | `GET /events/category/:category?page=&limit=` | `L` · `∅` « Rien de prévu en Théâtre pour l'instant » → « Voir tous les événements » · `404` for an unknown segment → `not-found.tsx` · `err` · segment is the **lower-cased** enum (`/categories/concert`) · refinement chips **deep-link to `/events?category=…&city=…`**, because this endpoint accepts only `page`/`limit` | P1 |
| **Login** | `/login` | public | **CSR** — no data to render; the shell prerenders so the form paints instantly on 3G | `POST /auth/login` → `{ accessToken, refreshToken, expiresIn, user{id,email,firstName,lastName,role} }` | idle · submitting · `401` invalid credentials · **`403` = email not verified** (`auth.controller.ts:188`) — its own state, routed to `/verify-email`, **never** « accès refusé » · `403` account deactivated · **`429` at 5 attempts / 15 min** with a timed re-enable · `?next=` resume restoring the pending checkout selection | **P0** |
...
| **Forgot password** | `/forgot-password` | public | **CSR** | `POST /auth/request-reset` | idle · submitting · **always-success** — the copy is byte-identical whether or not the address exists · **`429` at 3 / hour** | **P0** |
...
| **Legal** | `/legal/terms`<br>`/legal/privacy`<br>`/legal/refunds` | public | **SSG** — prose, no endpoint, no data | *(static)* | Three pages, `index, follow`, linked from the footer, from `/events/[id]` and from `/orders/[id]`.<br>⚠ **`/legal/refunds` does real work in the purchase flow**: it must state that the 6 % service fee is **not** refunded (`request-refund.handler.ts:56`), and it quotes the rate in prose — putting it inside the [`GET /config/public` blast radius](#6-screens-blocked-or-degraded-by-the-missing-get-configpublic) | **P0** |
...
| **Create event** | `/organizer/events/new` | `ORGANIZER` | **CSR** — a multi-step client form with local autosave; there is nothing to server-render | `POST /events` → creates a `DRAFT`<br>`POST /events/:id/image` | steps 1..n · draft autosaved to `localStorage` so a dropped connection never costs the form · **two-phase create** — the event must exist before an image can attach, so a failed upload must not lose the draft · uploading with progress and type/size validation · **one image per event, no gallery** · `400` validation · created → redirect to `.../ticket-types` · `403` · ⚠ the live « votre billet à 50 DT coûtera 53 DT à l'acheteur » arithmetic needs the commission rate — **[blocked by `GET /config/public`](#6-screens-blocked-or-degraded-by-the-missing-get-configpublic)**; interim constant, labelled « estimation » | **P0** |
...
| **Ticket types** | `/organizer/events/[id]/ticket-types` | `ORGANIZER` (owner) | **CSR** — mutating and owner-scoped | `GET /events/:id` → `ticketTypes[]` (**there is no separate list endpoint**)<br>`POST /events/:id/ticket-types`<br>`PUT /events/:id/ticket-types/:typeId`<br>`DELETE /events/:id/ticket-types/:typeId` | `L` · `∅` « Aucun tarif — un événement sans tarif ne peut pas être vendu » · create / inline edit · `salesStartDate` / `salesEndDate` window fields · each row shows `soldQuantity`, `availableQuantity`, `isSoldOut`, `isOnSale` **as returned**, never derived · delete confirmation, refused once `soldQuantity > 0` · `400` · `403` · ⚠ same live buyer-price arithmetic — **[blocked by `GET /config/public`](#6-screens-blocked-or-degraded-by-the-missing-get-configpublic)** | **P0** |
...
| **Event analytics** | `/organizer/events/[id]/analytics` | `ORGANIZER` (owner) · `ADMIN` | **CSR** — owner-scoped analytics | `GET /analytics/events/:id` → `{ totalRevenue, totalTicketsSold, totalCapacity, checkInCount, checkInRate, conversionRate, averageTicketPrice, topSellingTicketType, salesByDay[], checkInsByHour[] }`<br>`GET /analytics/events/:id/sales-timeline` | `L` · `∅` no sales yet, with the copy-link CTA · `err` · date-range selector · `403` · export via `POST /analytics/export` · ⚠ revenue is **gross** (gap 8) | P1 |
| **Check-in scanner** | `/organizer/scanner` | `ORGANIZER` · `ADMIN` | **CSR** — camera access requires the browser and a secure context; nothing here can be server-rendered. Dark `ink-950` surface, focus ring swaps to `sun-400`. Not event-scoped in the URL: the scanned ticket carries its own event | `POST /tickets/check-in` `{ qrCode, deviceId, locationGate }` | **setup** — `deviceId` (≤ 100 chars) and `locationGate` (≤ 50) are **required by the DTO**, so a one-time gate/device step must precede any scan; the device id is generated once and persisted · camera permission prompt / denied → **manual reference entry fallback** · scanning · **valid**: large success, holder name, tier · **invalid**, disambiguated: already checked in (with the timestamp), wrong event, cancelled, expired · **`429`** — the throttle is 3 req/s and a fast door queue will hit it: debounce and show a timed retry · `off` — ⚠ **check-ins cannot be queued offline**; the endpoint is the source of truth and the screen must say so plainly rather than pretend | **P0** |

‡ `/organizer/events/[id]` is P1 because the P0 cut redirects it to `.../edit`. It becomes P1 the moment an
organizer needs to *preview* rather than *edit*.
...
|---|---|---|---|
| Public | 11 | 11 (+ 3 static legal pages) | 5 SSR · 6 CSR |
| Participant | 9 | 9 | 0 SSR · 9 CSR |
| Organizer | 9 | 9 | 0 SSR · 9 CSR |
| Admin | 4 | 4 | 0 SSR · 4 CSR |
| **Total** | **33** | **33 (+3 static)** | **5 SSR · 28 CSR** |
...
| 5 | **Ticket** `/tickets/[id]` | An unusable ticket at the door | QR rendered client-side from a cached payload so it works offline, ≥ 240 px, brightness boost, PDF as explicit backup |
...
- **401** — silent refresh then replay; redirect only when the refresh itself fails, returning to the exact screen.
- **403** — see [§5.4](#54-the-three-meanings-of-403); the message differs by cause.
- **429** — action disabled with a visible retry timer; GETs back off automatically.
...
### 5.4 The three meanings of 403

`403` arrives from three unrelated causes and **must not share one handler**:

| Cause | Where | What the user must read |
|---|---|---|
| **Role failure** | Any role-guarded route | « Cette page est réservée aux organisateurs. » → route to their own home |
| **`IsEventOwnerGuard`** — organizer acting on someone else's event | `PUT/DELETE /events/:id`, ticket-types, image | « Vous n'êtes pas l'organisateur de cet événement. » → back to their event list |
...
## 6. Screens blocked or degraded by the missing `GET /config/public`

⚠ **`GET /config/public` is NOT IMPLEMENTED** — there is no config controller in `backend/src`.
`PLATFORM_COMMISSION_RATE` is read only inside `create-order.handler.ts`.

| Screen | Degradation | Interim behaviour |
|---|---|---|
| `/events/[id]` | Cannot quote an authoritative all-in price before an order exists | Indicative figure from `NEXT_PUBLIC_PLATFORM_COMMISSION_RATE`, labelled an estimate |
| Ticket selection sheet | Same | Same — reconciled against `platformFee` once `POST /orders` returns |
| `/checkout/[orderId]` | **Not degraded** — the order carries authoritative `subtotal` / `platformFee` / `total` | — |
| `/organizer/events/new`, `/edit` | Cannot show the organizer live « le participant paiera X » arithmetic authoritatively | Same constant, same caveat |

**Blast radius:** every pre-order price surface. The interim constant lives in **one module** so there
is exactly one place to correct, and it carries a comment naming the endpoint that will retire it.
This is a known-drift compromise, not a design — see [Phase 1 §L](02-product-design-brief.md#l-open-contract-questions-for-the-backend).
...
|---|---|---|
| 7.1 | No machine-readable error `code` | "Sold out" and "bad input" are both `400`; the UI keys off status + endpoint context in one `mapApiError()` shim |
| 7.2 | Sold out is `400`, not `409` | The sheet's conflict state is triggered by context, not status |
| 7.3 | `RATE_LIMITED` is `403`, not `429` | See [§5.4](#54-the-three-meanings-of-403) |
| 7.4 | `GET /events` returns `PUBLISHED` only | Admin moderation cannot see drafts; organizer event lists must use `GET /events/organizer/:organizerId` |
| 7.5 | `GET /events/organizer/:organizerId` is role-guarded | **No public organizer profile page is possible.** The event page shows the organizer's name and nothing more |
...
**P0 — the minimum coherent product (17 screens).** A participant can discover an event, buy a
ticket, and present it at the door; an organizer can create, publish and check in.

`/` · `/events` · `/events/[id]` · `/search` · `/login` · `/register` · `/verify-email` ·
`/forgot-password` · `/reset-password` · `/checkout/[orderId]` · `/checkout/[orderId]/retour` ·
...
`/participants` · `/analytics` · `/legal/*`

**P2 — administration and long tail (+4).** `/admin` · `/admin/reports` · `/admin/moderation` ·
`/admin/users` · `/unsubscribe/[token]/[category]`
...
│   ├── categories/[category]/page.tsx              # /categories/[category] SSR
...
| No error `code` field | Error mapping depends on a message-parsing shim | Backend |
| Analytics response shapes unverified against live data | Chart components may need reshaping | Frontend |
