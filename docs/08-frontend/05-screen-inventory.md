...
| **Event detail** | `/events/[id]` | public | **SSR**, `no-store`, **never ISR** — the most shared and most indexed page in the product, so it needs complete HTML and OG tags on the first byte; and availability is a per-fetch snapshot that a shared cache would falsify ([§1.4](#14-caching-and-revalidation)). Sends the bearer token when present, so an owner sees their own `DRAFT` | `GET /events/:id` → `EventDto` incl. `ticketTypes[]`<br>`POST /orders` — fired from the ticket-selection sheet; **creates the order and reserves the tickets in one call** (`create-order.handler.ts:137`), then routes to `/checkout/[orderId]` | `L` · `404` for a deleted or never-published event — **say so; do not imply a broken link** · malformed UUID → `ParseUUIDPipe` 400, rendered as `404` · `403` when an anonymous visitor hits a `DRAFT` → rendered as `404` · `CANCELLED`: full-width `danger` banner, purchase bar replaced by text, imagery desaturated · `COMPLETED` · per tier — available / limited (« Plus que 4 places ») / **sold out + « Vérifier à nouveau »** / not-on-sale with the real `salesStartDate` · sheet open · the three `POST /orders` refusals, all **inline on the sheet**: `403` `RATE_LIMITED` (5 commandes / heure, `create-order.handler.ts:54`), `400` `INSUFFICIENT_AVAILABILITY`, `400` `TICKET_LIMIT_EXCEEDED` (10 billets par événement) · no-image fallback | **P0** |
...
`403` on these routes means *this resource is not yours*, never a role failure. ⚠ The two `/tickets`
screens are the exception noted in [§10](#10-risks-and-open-questions): their controller wires the
placeholder guard and 401s regardless of the token.
...
`IsEventOwnerGuard` — so a non-owning `ORGANIZER` receives **403**, not 404. ⚠ The events and tickets
controllers currently wire the *placeholder* `JwtAuthGuard`, which 401s every request whatever the
token says; these screens cannot be tested end to end until that is fixed ([§10](#10-risks-and-open-questions)).

| Screen | Route | Role | Rendering + why | Primary endpoint(s) | Key states | P |
|---|---|---|---|---|---|---|
| **Organizer dashboard** | `/organizer` | `ORGANIZER` · `ADMIN` (read-only) | **CSR** — personalised analytics behind a JWT | `GET /analytics/dashboard?timeRange=&page=&limit=` → `{ organizerId, totalRevenue, currency, totalEvents, totalTicketsSold, averageCheckInRate, revenueTimeline[], events[], page, limit, total }` | `L` · **first-run `∅`** — no events is an onboarding moment, not an error: explain what publishing does, « Créer mon premier événement » · no-sales `∅` carrying the shareable event link · `err` · period selector offering **exactly `7d` / `30d` / `90d`** — the only values `DashboardQueryDto` accepts, default `30d` · ⚠ **`totalRevenue` is GROSS** and must be labelled « ventes brutes »; no payout logic exists, so never imply a net figure (Phase 1 §L gap 8) · ⚠ `EventAnalyticsDto` carries `eventId` with **no title** → hydrate from `GET /events/organizer/:organizerId` | P1 |
...
| **Event moderation** | `/admin/moderation` | `ADMIN` | **CSR** | `GET /events` (read)<br>~~`DELETE /events/:id`~~ — **unusable by an admin** | `L` · `∅` · `err` · `403` · ⚠⚠ **read-only in V1.** `DELETE /events/:id` is `@Roles('ORGANIZER')` + `IsEventOwnerGuard`, and neither guard grants an admin bypass (`roles.guard.ts:37`, `is-event-owner.guard.ts:78`), so an admin gets 403 on any event they do not own. `GET /events` also returns **`PUBLISHED` only** — the controller never forwards the DTO's `status` field — so drafts are invisible. Ship a « Signaler à l'organisateur » path and a visible, explained « suppression indisponible » state; **do not ship a delete button that always 403s** (§7.8) | P2 |
...
`PLATFORM_COMMISSION_RATE` is read in exactly one place, with a `0.06` fallback
(`create-order.handler.ts:41`). Ignore `config/payments.config.ts` and its rival `0.04`: the file is
absent from `ConfigModule.forRoot({ load: […] })` (`app.module.ts:32`), so nothing reads it.
**0.06 is the live default.**
...
| `/organizer/events/new`, `.../ticket-types` | Cannot show the organizer live « le participant paiera X » arithmetic authoritatively | Same constant, same caveat |
...
> 🔴 **P0's second hard blocker is the auth guard.** The Events, Tickets and Notifications
> controllers import `shared/infrastructure/common/guards/jwt-auth.guard.ts`, which only checks
> `request.user` truthiness — it never extends `AuthGuard('jwt')`, nothing populates `request.user`,
> and no `APP_GUARD` is registered (`app.module.ts` binds only `APP_FILTER` and two interceptors).
> Every authenticated route on those three controllers therefore appears to reject with `401`,
> including `GET /tickets` and `GET /tickets/:id` — so **a participant can pay and then never open
> the ticket they bought.** U-05, U-06, O-02–O-07, O-09, U-08 and U-09 are unreachable until it is
> fixed. Orders, Users and Analytics use the real guard and are unaffected, as is public discovery.
...
| **Events and tickets routes use a placeholder `JwtAuthGuard`** — `shared/infrastructure/common/guards/jwt-auth.guard.ts` never runs Passport, it only asserts `request.user`, and nothing populates it | Every protected route on those two controllers `401`s even with a valid token, so `/tickets`, `/tickets/[id]`, `/organizer/scanner` and every `/events` mutation cannot be exercised end to end until the guard is swapped for `modules/users/infrastructure/guards/jwt-auth.guard.ts` | Backend |
