...
> **`POST /orders` creates the order _and_ reserves the tickets internally** — step 5 of
...
mobile browser throttles timers. Renders **both forms at once** — « Il reste 12:34 » _and_
...
_verification in progress_ — **never as failure, and never with a retry button**, which is how double
